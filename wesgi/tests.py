...
class TestProcessInclude(TestCase):
    ...
    def test_match(self):
        ...
        self.assertEquals(mw.http.request.call_args, ((
    ...
    def test_recursive(self):
        ...
        # Even with the akamai policy, if we're not in debug mode, no error is
        # raised.
    ...
    def test_some_http_error_cases(self):
        ...
        self.assertEquals(
            mw.http.request.call_args_list,
            [(('http://www.example.com', ), ),
                (('http://alt.example.com', ), )])
        # onerror = "continue" has no effect if there is only
        # one error and alt is specified
        mw = make_mw()
        mw.http.request.side_effect = side_effect
        data = mw._process_include(
            'before<esi:include src="http://www.example.com" '
